...
def block_lu_gpu_device(
    ...
    snapshot_memory(memory_tracker, host_matrix, A, label="block_lu_gpu_device:start")
    ...
    for k in range(0, n, block_size):
        ...
        snapshot_memory(
            ...
            L11,
            U11,
            label="block_lu_gpu_device:after_panel",
        )
        ...
        snapshot_memory(
            ...
            L11_gpu,
            U11_gpu,
            label="block_lu_gpu_device:panel_gpu",
        )
        ...
        snapshot_memory(
            ...
            U11_gpu,
            U12,
            label="block_lu_gpu_device:after_u12",
        )
        ...
        snapshot_memory(
            ...
            U12,
            L21,
            label="block_lu_gpu_device:after_l21",
        )
        ...
        snapshot_memory(
            ...
            U12,
            L21,
            label="block_lu_gpu_device:after_schur",
        )

    cp.cuda.Stream.null.synchronize()
    L = cp.tril(A, k=-1) + cp.eye(n, dtype=A.dtype)
    U = cp.triu(A)
    snapshot_memory(memory_tracker, host_matrix, A, L, U, label="block_lu_gpu_device:final")
    return A, L, U
...
def solve_via_lu_gpu_device(
    ...
    snapshot_memory(memory_tracker, L, U, rhs_gpu, label="solve_via_lu_gpu_device:start")
    y = solve_triangular(
        L,
        rhs_gpu,
        lower=True,
    ...
    snapshot_memory(memory_tracker, L, U, rhs_gpu, y, label="solve_via_lu_gpu_device:after_forward")
    x = solve_triangular(
        U,
        y,
        lower=False,
    ...
    snapshot_memory(memory_tracker, L, U, rhs_gpu, y, x, label="solve_via_lu_gpu_device:final")
    return x


def solve_via_lu_gpu(
    L,
    U,
    rhs: np.ndarray,
    memory_tracker: AlgorithmMemoryTracker | None = None,
    ...
    return cp.asnumpy(solve_via_lu_gpu_device(L, U, rhs, memory_tracker=memory_tracker))
